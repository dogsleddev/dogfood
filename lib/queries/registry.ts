...
export const SCOUT_REGISTRY: readonly ScoutToolBinding[] = [
  ...
  {
    tool: "getMonthlyBalanceSheet",
    module: "balance-sheet",
    lane: "data",
    query: "getMonthlyBalanceSheet",
    wired: true,
    description: "The Balance Sheet broken out MONTH BY MONTH for the fiscal year containing `period` (each cell is that line's month-END balance; the Total column = the FY-end balance). Use for a monthly balance-sheet trend or any 'balance sheet by month / which month' question (e.g. cash, AR, or deferred-revenue trajectory). For the whole-year point-in-time Balance Sheet use getBalanceSheet.",
  },
  {
  ...
  },
  {
    ...
    write: true,
    wired: true,
    description: "WRITE: create a new (empty) user scenario with a name + baseline ('base' or 'budget'). Group-scoped, recorded on the user's behalf. Use when the user says 'create/make/start a scenario'. Then add levers with setDriver and read it with getScenarioPnL. To branch from an existing scenario instead, use duplicateScenario.",
  },
  {
    tool: "duplicateScenario",
    module: "scenarios",
    lane: "data",
    query: null,
    write: true,
    wired: true,
    description: "WRITE: copy any scenario (Base, a preset like 25% Profit, or a user one) into a fresh editable user scenario (pass sourceId). Use to branch off an existing scenario — and the ONLY way to edit Base or a preset (they're immutable; duplicate then setDriver on the copy).",
  },
  {
    tool: "setDriver",
    module: "scenarios",
    ...
    write: true,
    wired: true,
    description: "WRITE: add one driver adjustment to a USER scenario (lever + magnitude + window + step/ramp). Levers: revenue/direct_cost take a % (25 = +25%), personnel/expense a $/mo delta, ar_dso days (ap_dpo gated). Engine-validated against the forecast horizon. This is how you MODEL a what-if: create/duplicate a scenario, setDriver, then read the result with getScenarioPnL — NEVER hand-compute the hypothetical off getPnL. Base + presets are immutable (duplicate first).",
  },
  {
    tool: "resetScenario",
    module: "scenarios",
    ...
    write: true,
    wired: true,
    description: "WRITE: clear all adjustments on a user scenario (reset it to its baseline). The undo for setDriver. Rejects Base/presets.",
  },
  {
    tool: "deleteScenario",
    module: "scenarios",
    ...
    write: true,
    wired: true,
    description: "WRITE: delete a user scenario entirely. Rejects Base + presets (they're immutable code constants).",
  },
