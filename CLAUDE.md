...
**Update (2026-06-23) — DEPLOYED + LAUNCH-HARDENED.** The app is **live at [dogfood.cafe](https://www.dogfood.cafe)** (Vercel; GitHub auto-deploy; the site is open — no login to view / use Scout / add flux notes / build scenarios — while app-wide "data" writes are admin-gated at `/admin`; a daily self-heal cron resets the shared sandbox). Since the 2026-06-22 status: the **CSV importer + the detail-to-TB reconciliation control total** (on Setup → Data Import, the Loom credibility centerpiece) and a **writable global as-of** are built; **Scout's full scenario WRITE surface** (create/duplicate/setDriver/reset/delete) is wired; and the **public `/api/scout` endpoint is rate-limited** (per-IP + a global spend circuit-breaker, Supabase-backed, fail-open, admin-exempt). **This session (2026-06-23):** mobile-native Scout + a read-only mobile dashboard home; the `getFluxDetail` FY-vs-month tie-out fixed (the variance drill now reconciles on camera — month actual === Σ its bills); and a launch-hardening small-wins batch (`cac_payback`→`cac_per_logo` metric-id fix, a Scout runway/burn nudge to lead with the cash-burn bridge + the annual-prepay explanation, a shared-sandbox banner, 7 guides). Gates green: data-sweep 44/44, scout-readiness 78/78, scout-eval LLM 70/70, live smoke 57/57. **The pre-Loom launch-hardening is complete; the next step is recording the Loom.** Remaining code is P2/optional (the two config-tool wirings `getPeriodConfig`/`getExpenseGroups`, per-session anon-write scoping, `lockBudget(source=scenario)`, traffic-gated mobile tiers). `Handoff.md` has the exact next steps.
