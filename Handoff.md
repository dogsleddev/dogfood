...
**Three of the four documented next steps landed, each independently gated. The CSV importer is DESIGNED-IN-FULL (a 4-phase design+impact workflow, output `wf_e086eecf-da5`: 3 proposals × adversarial verify × synthesis) and its as-of + validate foundation is BUILT; the reconcile + UI are the next slices, turnkey from the locked design below.**
...
**▶ NEXT — finish the CSV importer (the locked design; slices 1b → 2 → 3), then a decision for Chris.**
- **Slice 1b — reconcile + advanceClose orchestrator (headless, the importer payload).** `lib/import/reconcile.ts`: per (account, period), compare the imported-TB MOVEMENT to the generator's GL activity. **P&L accounts reconcile FYTD-to-FYTD** (Σ generator activity `[fyStart..periodIdx]`); **BS accounts balance-to-balance** (opening + Σ activity `[0..periodIdx]`) — getting this split wrong false-flags every P&L account (the verdicts' subtlest-bug warning). Cleanest: export a `accountTrialBalanceAt(period): Map<code, naturalAmount>` helper FROM `lib/seed/gl.ts` (it already has `openingByCode` private + the activity map — reuse the exact opening literal, no duplication) and compare imported vs it. Threshold `max($1, 0.1%·line)`. A gap > threshold is a BLOCKING `needsAttention` finding (no plug, fix-upstream). `lib/queries/import.ts` orchestrator: parse → validate → reconcile → if clean AND period === the in-close month → `store.advanceClose(period)`; a clean prior-month re-import is a restatement no-op (green reconcile, as-of unmoved). Extend `importer-check` with the standing tripwire: the real May TB reconciles to **$0.00 across all 28 accounts** (it IS generator truth). Persist an `import_runs` audit row (migration `0007`, EXCLUDE from the seed-loader clear array; settings → upsert-if-absent so a re-seed never un-advances the as-of — verified breaker at `seed-supabase.ts:82`) can ship with this slice or with the UI.
- **Slice 2 — the UI.** Replace the `app/setup/data-import/page.tsx` PlaceholderPage with a Server-Action upload (mirror the `?edit=` idiom, no client JS): drop `trial_balance.csv` → validation + per-account reconciliation exception list with $ gaps → "Commit & advance as-of" gated on a clean reconcile → `import_runs` history. Verify live (preview): June advances, statements/board/Scout flip June to Actual, §11 unmoved.
- **Slice 3 — COA base-import (deferred tier).** `chart_of_accounts.csv` re-points `statement_line` at a new `coa_base_import` tier, composed `composeGlAccounts(composeGlAccounts(CHART, coaBaseImport), userOverrides)` (override-check stays 44/44 at empty by reference). Same-group rows auto-apply; cross-group / new-code / accountType-nature changes → BLOCKING needs-attention, never silently dropped.
- **DECISION FOR CHRIS (the one design sign-off): the TB STORAGE OVERLAY.** Should the importer also let a TB whose numbers DIFFER from the generator MOVE a closed month's Actual (a true historical restatement engine), or stay validate+reconcile+advance only (generator is truth; a contradicting import is a fix-upstream flag)? **Recommendation: DEFER the overlay.** It introduces a second source of truth, requires re-routing Cash Flow NI Actual through an overlay-aware path (else CF silently ignores the restatement while the P&L honors it — confirmed at `statements.ts:446`), needs a balanced-month guard, and has zero coverage from the existing 44 data-sweep checks — real tie-out risk for a capability Bearing's real TB (== generator truth) never needs. Deferring is the §16-faithful behavior (a contradicting import is fix-upstream, not absorbed). Defer behind a clean `TBOverlayStore` seam.

**New files this session:** `lib/import/{parse-csv,types,validate}.ts` · `scripts/{importer-check,importer-asof-check,flux-repoint-check}.ts`. **HOW TO START 1b:** read this block + the locked design above + `CLAUDE.md` §16; the as-of advance is already proven safe, so 1b is the reconcile math + orchestrator. data-sweep MUST stay 44/44; §11 unmoved.
