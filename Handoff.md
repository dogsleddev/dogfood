...
### State (2026-06-23 · END OF SESSION · LAUNCH-HARDENING COMPLETE — next = the Loom) — GREEN  ·  _authoritative; the blocks below are this session's per-deploy detail + prior context_

**END-OF-SESSION ROLL-UP (2026-06-23).** A 5-reviewer progress review reframed the work to LAUNCH-HARDENING (the build is feature-complete: all 25 surfaces + engine + Scout + mobile live). This session shipped + DEPLOYED + verified-live, in order: (1) **mobile-native Scout** (full-screen sheet on phones — recovered from a mid-work crash that opened the session); (2) **Mobile v1** (read-only dashboard home + Scout); (3) **the Scout rate-limiter** (the review's #1 — detailed below); (4) **the `getFluxDetail` FY-vs-month tie-out** (`906785d` — the on-camera drill reconciles); (5) **a launch-hardening small-wins batch** (`611d89d` — `cac_per_logo` id fix, runway/burn nudge, shared-sandbox banner, 7 guides). **Every pre-Loom launch-hardening item the review flagged is now closed.** Gates green throughout: data-sweep 44/44 · scout-readiness 78/78 · scout-eval LLM 70/70 · peer-check §11 unmoved · live smoke 57/57. Git: clean, `local == origin/main`. **▶ TRUE NEXT = record the Loom** (no code needed); remaining code is all P2/optional (see the ▶ NEXT list below). The per-deploy detail for each item is in the blocks that follow; the rate-limiter (the review's #1) is detailed immediately below.

**CONTEXT (the rate-limiter — the review's #1): the public `/api/scout` endpoint ran the agent loop server-side with NO per-user auth / no cap → a spike = uncapped Anthropic spend + 529s. Migration 0008 applied to live Supabase. Detail:**
...
**▶ NEXT (the review's remaining launch-hardening, in order):** (1) ~~fix the `getFluxDetail` FY-vs-month tie-out~~ **DONE this session (`906785d`)** — the drill now leads with `monthActual` (the line's actual for that month, ties to the bills) + an explicit `fullYear` context block; defaults to the last closed month; gate `flux-repoint-check` asserts `monthActual === Σ transactions` ($37,296 IT/May) and `≠ FY actual`. scout-eval LLM 70/70, scout-readiness 78/78. (2) ~~pin the canonical arc~~ **DONE/no-op** — verified no stale figures in `lib/` (109%/84-16/49mo already throughout; the stale 111%/77-23/38mo was only in the review brief). (3) ~~cash-burn-bridge Loom nudge~~ **DONE (`611d89d`)** — Scout's system prompt now leads runway/burn answers with `getCashBurnBridge` + the annual-prepay "growth collects cash up front" explanation. (4) **anon writes → SHIPPED the shared-sandbox banner** (`611d89d`, on Scenario Manager); per-session scoping DEFERRED (a bigger write-path refactor, not worth shipping mid-spike — the banner sets expectations). (5) small-wins: **cac_payback → cac_per_logo DONE** (`611d89d`, the 136-mo-payback footgun; export regenerated) + **guide-count DONE** (7 not 6); **DEFERRED: wire `getPeriodConfig`/`getExpenseGroups`** (adds 2 NEW tools to the live Scout agent — deserves its own focused eval pass, not a mixed batch into the live window) + **`lockBudget(source=scenario)`** (admin-only, bigger build). Full review synthesis: workflow `wf_10046cd4-2a1`; the rate-limit adversarial review: `wf_750c586b-14e`.

**▶ TRUE NEXT: record the LOOM.** All pre-Loom launch-hardening the review flagged is now DONE + deployed (Scout rate-limited, flux drill reconciles on camera, reconciliation control total ties to $0, runway bridge + Scout nudge, mobile, canonical arc consistent). The remaining code (config-tools wiring, per-session scoping, lockBudget-scenario, traffic-gated mobile tiers) is all P2/optional — don't let it delay the Loom. Re-run `npm run smoke` + a Scout sample immediately before recording.
...
**Shipped this session (all gated):**
- **Reconcile engine — `lib/import/reconcile.ts`.** `DETAILED_ACCOUNTS` = the 13 accounts with a sub-ledger: **1100 AR** (Σ invoices − Σ receipts, cumulative — incl. the opening invoices + negative credit memos), **4100** services revenue (timesheets billable value), **5000 / 6000** payroll (paychecks split by `staff.function === 'direct'`), **5100 + 6100-6800** (vendor bills by posting account). Each reconciles Σdetail vs the TB within materiality `max($1, 0.1%·line)`; over → blocking "needs attention", no plug, fix-upstream. The other **15 accounts are TB-ONLY** (equity, deferred revenue, AP, D&A, SBC, cash, taxes, **subscription revenue** — recognized ratably, no per-txn event): authoritative, nothing to reconcile up, so they can NEVER throw a false variance (the derived-equity / closing-entry trap, §16). The engine reads the generator directly (deterministic, identical on both backends); the reconcile is account-grain so an Account-Mapping re-point doesn't move it (only the displayed `statementLineId` reads the effective map).
- **`accountTrialBalanceAt(period)` exported from `lib/seed/gl.ts`** — the TB side, natural balance: P&L = FYTD activity Σ[fyStart..i]; BS = opening + Σ activity[0..i]; **accumulated deficit (3100) = the PRIOR-FY-END balance** (the summary GL posts no closing entry to it, so a TB with open P&L carries equity at prior-FY-end — that is exactly what foots the books: Assets + Σexpense-FYTD + AccDef[priorFY] === Liab + PaidIn + Σrevenue-FYTD). `isDebitNormal` also exported (the importer converts an imported TB row's debit/credit to the same natural balance).
- **Orchestrator — `lib/queries/import.ts`.** `getReconciliation(period)` / `getDataImportView()` (the standing control total over the seed's own books) and `runTrialBalanceImport(csv)` (parse → validate → reconcile against the imported TB → commit): a clean reconcile of the **next in-close month** advances the global as-of (it flips to Actual everywhere); a closed-month re-import is a restatement no-op; a gap or a validation failure is recorded and blocked. Every path records an `import_runs` audit row.
- **`import_runs` audit — migration `0007_import_runs.sql`** (folded into 0001) + `recordImportRun` / `listImportRuns` on the DataStore (interface + in-memory). The **Supabase override falls back to the inherited in-memory array** when the table is absent (regex on the PostgREST error), so the importer works WITHOUT applying 0007 (the run is then session-local). Seed-loader change: `settings` is now **upsert-IF-ABSENT** (a re-seed preserves an advanced as-of) and `import_runs` is excluded from the clear array (both are write surfaces).
- **The visible surface — `app/setup/data-import/page.tsx`** (replaced the PlaceholderPage; Server Actions, no client JS). A standing reconciliation control-total table (TB / detail / variance / ✓Tied per account, rolled to its statement line; a `<details>` for the TB-only authoritative accounts) + a green/ember status banner + a TB **file upload** + **3 live-generated demo buttons** (`lib/import/sample-tb.ts`: next-month = clean → advances; broken = contradicting → exception; current = restatement) + the import history. **Added the missing Data Import nav link** (`components/nav/nav-config.ts` — it had no nav entry). `scripts/reset-asof.ts` rolls the as-of back after a demo advance.
- **2 latent bugs the importer SURFACED + fixed** (exactly the kind of inaccuracy the Loom must not show): (a) the committed `import-templates/trial_balance.csv` was **STALE** — its payroll split predated the exec-comp / eng-org recalibrations, so importing it showed a false **$26K** exception on 5000/6000. Regenerated via `scripts/make-trial-balance.ts` (one source with the live demo samples); new foot **$59,587,121.23** (was 59,567,335.23). importer-check now guards the contract TB against generator drift (forces a regen if the seed moves). (b) `accountTrialBalanceAt` returned **$0 for accumulated deficit** (no GL activity posts to 3100) → the prior-FY-end fix above.

**Gates (all green):** tsc/lint 0 · `next build` OK (`/setup/data-import` compiles) · data-sweep **44/44** · **importer-check PASS** (now also: the seed's books reconcile to $0 across all 13 detailed accounts at multiple FYTD boundaries · a contradicting TB blocks on the right account with no false positives · the orchestrator restatement + **commit-advance** paths · degenerate-input rejects (header-only / multi-period) · the contract-TB-vs-generator drift guard) · **importer-asof-check 12/12** · flux-repoint-check 5/5 · scenario-check 20/20 · **scout-readiness 76/76** · **scout-eval LLM 69/69** (the +2 = the new getReconciliation cases). **Verified LIVE on Supabase** (preview): the page shows "Books reconciled — $0 variance · all 13 ✓ Tied," the broken demo lights up a needs-attention banner + history row with the as-of unmoved, the next-month import advanced May→June + reconciled at June then `reset-asof` restored May, 0 console errors.

**Adversarial review (workflow, 6 dimensions × adversarial-verify → 9 confirmed) — the real findings FIXED + gated:**
- **[P1] Multi-period TB only reconciled the latest period** — a contradiction in an earlier month bundled with a clean later month bypassed the reconcile and could silently advance the as-of. **Fixed:** the orchestrator now requires EXACTLY ONE period per import (the locked one-TB-per-month contract); 0 = empty file, >1 = multi-period → both REJECTED.
- **[P2] Empty/header-only TB reported a false green "Reconciled"** (vacuous reconcile against a NaN "—" sentinel). **Fixed** by the same single-period guard (the empty case is now `Rejected: no trial-balance rows found`).
- **[P2] The import Server Action had no try/catch** — a Supabase store error (constraint/RLS/network) would 500 the page. **Fixed:** wrapped; an unexpected throw is best-effort recorded as a `rejected` run and the page re-renders.
- **[P2] supabase-parity's getSettings compare self-masked** a persisted-as-of divergence (mem's live getter object serialized after Supabase read-repaired the global boundary). **Fixed:** snapshot the in-memory settings to a plain object before reading Supabase.
- **[nit] TB-only disclosure** now names subscription revenue (the primary trusted-without-back-check line).
- Two tripwire cases added to importer-check (header-only + multi-period → rejected). **Not changed** (verified low / non-issues): the import_runs fallback durability (bounded to a misconfigured multi-instance deploy; already flagged to apply 0007) and the "$0 variance" banner (the standing banner only reflects the gate-clean seed books, so it's accurate in the shipped state).

**Loom-ready pass (Chris's pick after the control total) — Scout reconciliation tool + live advance + guide truthfulness:**
- **Scout `getReconciliation` tool — WIRED + gated** (Scout Follows Modules: the reconciliation module shipped → its tool is wired; the query already existed). Registry binding (`module: config`), impl in `lib/scout/tools.ts` (concise shape — reconciled? · detailed/authoritative counts · exceptions with account + variance · receipt → `/setup/data-import`), a RECONCILIATION system-prompt section, a deterministic-router route + summarize case, a commentary label, and 4 eval cases (2 deterministic + 2 LLM, incl. the "tie out" → getReconciliation NOT getBalanceSheet disambiguation). The demo beat: *"Ask Scout: are the books reconciled?"* → a grounded answer + click-through receipt. **scout-eval LLM 69/69** (was 67) · **scout-readiness 76/76** (was 74 — the tool executes via the readiness thunks).
- **Live advance verification on Supabase — the one seam the gates don't cover (they force in-memory).** Imported the next-month TB on the live app → the as-of persisted May → June (`advanceClose` → Supabase settings write → `getSettings` read-repair → re-render), the standing control total recomputed + reconciled at June, then `scripts/reset-asof.ts` restored May (confirmed by a fresh-process getSettings read). NOTE: an EXTERNAL reset doesn't bust Next's cache, so a running server needs a restart/re-trigger to reflect it (the import action itself revalidates).
- **Guides truthful** — `lib/guides/content.ts` (the updating-actuals / new-period guide, Scout's product-knowledge lane) + the `config.ts` corpus now describe the shipped detail-to-TB control total + the import → reconcile → advance flow, so Scout answers "how do I close a month / are the books reconciled" truthfully.

**Thorough QC + reconciliation pass (post-ship) — full suite green, Scout stress-tested, 8 findings found + ALL FIXED:**
- **Every gate green together:** tsc/lint 0 · `next build` 40/40 · data-sweep **44/44** · peer-check §11 ✓ · importer-check PASS · importer-asof 12/12 · override-check 44/44 · flux-repoint PASS · scenario-check 20/20 · scout-readiness 76/76 · supabase-parity PASS · schema-check PASS.
- **Scout 100-question stress test (`scripts/scout-100.ts`, NEW)** — 106 questions across 21 categories through the REAL LLM loop: **routing 106/106 (100%)** and **grounding 24/24 (100%)** — for every numeric answer, the figure Scout quoted was cross-checked against the same tool's returned data ("the report") and matched. No misroutes, no hallucinated/dropped numbers.
- **8-dimension review workflow** (code · data · routes · datastore/schema · Scout · code-quality · docs · security) × adversarial verify → 8 confirmed findings, all FIXED:
  - **[P1] no-key router: "cash flow" returned the Balance Sheet** — `\bcash\b` (route to getBalanceSheet) shadowed the `/cash flow/i` route; keyless trial users hit this. Fixed: `\bcash\b(?!\s*flow)` + a DET_CASES regression guard. (Same class, nit: month-qualified "burn" now reaches the monthly CF trajectory.)
  - **[P1×2] stale Scout corpus** — the *creating-scenarios* + *asking-scout* guides said user scenario create/edit and Scout's what-if/setDriver weren't shipped yet; they ARE (and config.ts tells Scout to use them). Rewrote both so Scout's product-knowledge lane matches the live write surface.
  - **[P2] `accountTrialBalanceAt` didn't FOOT for FY2024** — the pre-window opening accumulated deficit ($8.27M) was dropped at fy0=0, so a generated FY2024 TB overstated Cash by $8.27M (latent: unreachable via any shipped path — everything defaults to 2026-05). Fixed (source the FY2024 prior-FY-end deficit from `opening.accumulatedDeficit`) + a **cross-FY footing tripwire** in importer-check (the gate only footed the close month — exactly why it slipped); now foots Δ$0.00 at 2024-01/06/12, 2025-12, 2026-05.
  - **[nits] fixed:** "19 dashboard metrics" comments → 20; CLAUDE.md §11 non-GAAP/FCF margins → −33%/−17% (live); import-templates/README build-status header → "shipped + live". Data tie-out, schema, and app routes came back clean.

**New files:** `lib/import/{reconcile,sample-tb}.ts` · `lib/queries/import.ts` · `components/setup/reconciliation-table.tsx` · `app/setup/data-import/{page,actions}.tsx` · `supabase/migrations/0007_import_runs.sql` · `scripts/{make-trial-balance,reset-asof,schema-check,scout-100}.ts` · `import-templates/trial_balance_{next-month,broken}.csv`. **Changed:** `lib/seed/gl.ts` · `lib/import/types.ts` · `lib/datastore/{datastore,in-memory,supabase}.ts` · `scripts/{importer-check,seed-supabase,supabase-parity}.ts` · `components/nav/nav-config.ts` · `import-templates/trial_balance.csv` (regenerated) · **Scout tool:** `lib/queries/registry.ts` · `lib/scout/{tools,config,router,commentary}.ts` · `scripts/scout-eval.ts` · `lib/guides/content.ts`.

**🚀 LIVE ON `dogfood.cafe` `[2026-06-22]` — deployed to Vercel + the domain swapped + admin-gated.** The app is **OPEN** (no login — trial users view everything, add flux notes, build scenarios, use Scout; all contained, never moving Base/actuals). The Basic-Auth gate was REMOVED (commit `83fa3e7`) per Chris. **ADMIN GATE (commit `0e286cb`):** the app-wide "data" writes — CSV import / as-of advance, Account Mapping re-point, Budget lock/reset, and the demo reset — require admin sign-in at **`dogfood.cafe/admin`** (password **`Rainman1`**, env `ADMIN_PASSWORD`; signed httpOnly cookie via `lib/auth/admin.ts` `isAdmin()`). The Data Import controls are HIDDEN for non-admins (the reconciliation control total stays visible to all). **SELF-HEAL:** `resetDemo()` (`lib/queries/admin.ts`) clears the user-write tables (scenarios / flux notes / account overrides / budget) + resets the global as-of to baseline — runs **daily via Vercel cron** (`/api/reset` 07:00 UTC, `CRON_SECRET`-protected, `vercel.json`; registered + enabled) **+** an admin "Reset demo" button on Data Import. Verified live: anon → site open + import hidden + "admin-only" note; valid admin cookie → import + reset visible; `/api/reset` no-secret → 401. apex `dogfood.cafe` 308→`www.dogfood.cafe` (primary); reconciliation renders "Books reconciled — $0 variance"; Scout API 200. **Vercel project `dogfood-app`** under the **dogsled TEAM** (`team_i1Es1eTRb83TisgbHEU6gcA5` — NOT the personal account; same-name slug collision, so use the team ID for `--scope`; CLI auth lives at `%APPDATA%\xdg.data\com.vercel.cli\auth.json`). GitHub `dogsleddev/dogfood` is connected → **pushes auto-deploy**. 7 prod env vars set. The old `dogfood` landing project (git-connected to a `doghair` GH account) was deleted after the domain moved off it. **TWO DEPLOY GOTCHAS (both fixed, documented for next time):** (1) **the project Framework Preset MUST be `nextjs`** — a blank `vercel project add` leaves `framework: null` → Vercel treats it as "Other", IGNORES the `.next` output, and serves only `public/` + a 404 catch-all (EVERY route 404s). Set via `PATCH /v9/projects/{id}` `{"framework":"nextjs"}`. (2) **`@supabase/supabase-js` pulls in `ws` (Realtime), which uses `__dirname`** → "ReferenceError: __dirname is not defined" in the bundled serverless functions → fixed in `next.config.ts` (`serverExternalPackages` + a webpack `externals` for ws/bufferutil/utf-8-validate; commit `a79330b`). The `__dirname` error first PRESENTED as an edge-middleware failure, but the real root cause was the missing framework preset (the broken "Other" build produced a broken middleware bundle).

**✅ SHIPPED `[2026-06-22]` — committed + PUSHED to `origin/main` (`a79330b`), the prior held commits flushed with it; migration `0007` APPLIED to live Supabase.** `git` is in sync (0/0). **`scripts/schema-check.ts` (NEW)** verifies the LIVE Supabase schema matches the migration files (the structural-soundness gap that bit us twice — stale `subCode`, unapplied `import_runs`); it found `import_runs` missing → applied 0007 via `scripts/apply-schema.ts supabase/migrations/0007_import_runs.sql` → **schema-check now PASS** (every contract table + load-bearing column present). **Apply/check both need the Session-pooler env on IPv4** (the direct host is IPv6-only): `SUPABASE_DB_HOST=aws-1-us-west-2.pooler.supabase.com SUPABASE_DB_USER=postgres.zdbuojhtfvdbanhxedwx SUPABASE_DB_PORT=5432 npx tsx scripts/schema-check.ts`.
- **⚠ Vercel deploy serves only if the project env is set:** `NEXT_PUBLIC_SUPABASE_URL` · `NEXT_PUBLIC_SUPABASE_ANON_KEY` · `SUPABASE_SERVICE_ROLE_KEY` · `DATASTORE=supabase` · `ANTHROPIC_API_KEY` (Scout) · **`BASIC_AUTH_USER` + `BASIC_AUTH_PASSWORD`** (the middleware is fail-closed — no creds → 503, never public). Confirm these on Vercel; the push auto-deploys.
- The **advance demo persists the as-of to Supabase** — `npx tsx scripts/reset-asof.ts` rolls it back to May (an external reset needs a server restart/re-trigger to show through Next's cache).

**▶ NEXT (Chris's review → tweaks): the "only I change the full app" model.** Gate the THREE app-wide writes behind an admin marker so trial users can't trigger them: the **import / as-of advance**, an **Account Mapping re-point** (moves the Actual columns), and a **Budget lock/reset** (moves the Budget column). Leave the CONTAINED writes open + shared (Chris OK'd this): **scenarios** (incl. Scout's writes — never touch Base/actuals) and **flux notes** (annotations, change no figure). There is no edit-the-base-forecast surface — a forecast change is a scenario (contained). This also removes the as-of concurrency worry (only Chris advances). NOT multi-tenant, NOT slice 3 (deferred).

**▶ NEXT (importer): slice 3 — COA base-import (deferred / lower priority).** `chart_of_accounts.csv` re-points `statement_line` at a `coa_base_import` tier, composed `composeGlAccounts(composeGlAccounts(CHART, coaBaseImport), userOverrides)`; same-group rows auto-apply, cross-group / new-code / accountType-nature changes → BLOCKING needs-attention. Other open write paths remain (the TB storage-overlay stays DEFERRED behind a `TBOverlayStore` seam — accuracy comes from the control total, not from imports rewriting Actuals). The full locked importer design is workflow output `wf_e086eecf-da5`.

### State (2026-06-22 · F4 + Scout full scenario WRITES + CSV importer foundation [as-of seam + validate]) — GREEN  ·  _prior context_

**HANDOFF TO A NEW SESSION (Chris's call — context budget). Three of the four documented next steps landed, each independently gated; the CSV importer is DESIGNED-IN-FULL and its as-of + validate foundation is BUILT. Chris then LOCKED the importer's source-of-truth + control-total model (see ▶ NEXT): TB = single source of truth, a visible detail-to-TB reconciliation control total ON Setup → Data Import (the Loom centerpiece), overlay DEFERRED. The reconciliation control total is the next session's first build.**

**Shipped this session (all gated):**
- **F4 (deferred flux fix) — DONE.** `listFluxNotes` (in `lib/queries/flux.ts`) now resolves an account/transaction-anchored note's EFFECTIVE statement line from the CURRENT map (chart ⊕ overrides) when filtering by `statementLine`, instead of the line denormalized at write time — so a re-pointed account's note follows to its new line on the statement peek pane (it was stale). Notes anchored directly to a line (grain 3) keep their stored line. Done in the QUERY layer (the DataStore stays a dumb store; works on both backends). Gate: `scripts/flux-repoint-check.ts` (5/5).
- **Scout FULL scenario WRITE surface — DONE (resolves the long-open §17 item).** `setDriver` unblocked + the whole authoring surface wired: `createScenario` · `duplicateScenario` · `setDriver` · `resetScenario` · `deleteScenario` (all `write:true`, recorded on the user's behalf, engine-validated, never touching Base/actuals). New query-layer write fns in `lib/queries/scenarios.ts` (`createScenario`/`duplicateScenario`/`resetScenario`/`deleteScenario`/`setScenarioDriver` + `DriverInput`/`ScenarioWriteResult`) wrap the registry + `validateScenario` (the shared spine the UI Server Actions also use). Tools in `lib/scout/tools.ts`, bindings in `registry.ts`, commentary, and the system prompt rewritten (a what-if now routes THROUGH the engine — create/duplicate → setDriver → getScenarioPnL — NEVER hand-computed off getPnL). The no-key router keeps a graceful what-if decline (it can't parameterize a write). Eval: the 2 old "refuse" cases flipped to "engage the engine" + 3 new write cases → **scout-eval LLM 67/67** (was 64/64); scout-readiness 74/74 (writes excluded from the readiness thunks).
- **CSV importer — slice 0 (parse + validate) + slice 1a (the as-of seam = the old "as-of Phase B") — DONE.**
  - **Slice 1a (the keystone):** the global close boundary is now a MUTABLE behind getters. `lib/target/placeholder.ts` holds `_closeBoundary` + `getCloseBoundary()`/`setCloseBoundary()`; `PLACEHOLDER_SETTINGS.closeThrough`/`inCloseMonth`/`forecastHorizon` are GETTERS, so every call-time property-access reader (fyWindow, monthlyColumns, renewals NOW_IDX, the queries, Scout, the pages) auto-tracks the single source — NO 24-site rewrite needed. The ONLY frozen capture converted: `SEED_BUDGET_PERIOD` (a module-load const) → `seedBudgetPeriod()` function (updated the 2 in-memory usages). `DataStore` gained `updateSettings(patch)` + `advanceClose(period)` (atomic 3-field write; rejects restatement / cross-FY / past-Dec-2026); `SupabaseDataStore.getSettings` READ-REPAIRS the mutable from the persisted `settings` row + an `updateSettings` override persists it. **Gate `scripts/importer-asof-check.ts` (12/12):** drives `setCloseBoundary` to June and proves §11 anchors (rev 23,420,700 / NI −11,918,577 / runway 49mo) byte-identical at both boundaries, FY Forecast byte-identical, June's activity flips forecast→Actual (rev Actual 8.29M→10.09M), the monthly board June cell value unchanged + status flips in_close→actual, the P&L rollup ties at June (17 leaves Δ $0.0044), and reset→May is byte-for-byte. data-sweep stays **44/44** at the default (May) boundary (the refactor is byte-identical there).
  - **Slice 0:** `lib/import/{parse-csv,types,validate}.ts` — a dependency-free RFC-4180 parser (quoted commas / BOM / CRLF) + `validateTrialBalance` (Σdebit===Σcredit per period to the penny, known + mapped accounts, one-row-per-(period,account), period format, optional past-horizon). Gate `scripts/importer-check.ts`: the REAL `import-templates/trial_balance.csv` parses (28 accounts), foots to **$59,567,335.23** each side, validates clean; a broken TB flags unbalanced/unknown/both-sides.

**Gates (all green):** tsc/lint 0 · `next build` OK · data-sweep **44/44** · **importer-check PASS** · **importer-asof-check 12/12** · **flux-repoint-check 5/5** · scout-readiness 74/74 · scout-eval **LLM 67/67** · (scenario-check / peer-check / supabase-parity unaffected — no seed/§11 change; re-run before push if touched).

**⚠ COMMIT STATUS — 3 commits are LOCAL-ONLY, NOT pushed (Chris chose "hold local").** On `main`: `c79e78d` (F4) · `36abe20` (Scout scenario writes) · `5397c1e` (importer foundation). `origin/main` is still at the prior QC push. The next session is on the same machine (`C:\dev\dogfood`), so the commits are present locally; **push when Chris OKs** (`gh auth setup-git` first — the credential-hang gotcha).

**▶ NEXT — build the RECONCILIATION CONTROL TOTAL (DECIDED with Chris 2026-06-22). This is the Loom centerpiece** (Chris is recording a demo; 50-100 people will trial the app over 3 days — they probably won't import, but the tie-out must be visibly, provably accurate). `[LOCKED]`

**The decided model (Chris's framing, = §16):**
- **The trial balance is the single source of truth** — and already is at read time: post the override-layer slice, the statements' Actual rolls up from the GL at the account grain (`activityByStatementLine` → statement line), which IS the trial balance. The drivers are only how the seed was generated; what the statements PRESENT is the TB (it balances on its own — `importer-check` proves the real one foots to $59,567,335.23 to the penny). Never a plug.
- **The control total = the detail-to-TB back-check** for accounts that have a SUB-LEDGER (expenses↔vendor bills, revenue↔invoices, payroll↔paychecks, AR↔receipts): `Σ(detail for account, period) === the TB movement`, with a signed variance + a Σ|variance| **control total** + a BLOCKING "needs attention" flag over a materiality threshold (`max($1, 0.1%·line)`). **Accounts with no sub-ledger (equity 3000/3100, D&A, manual JEs) are TB-ONLY** — the TB is authoritative, nothing to reconcile up, so they can NEVER throw a false variance (this sidesteps the derived-equity/closing-entry trap that would otherwise false-flag 3100). The gates ALREADY prove `Σ(detail)===GL` for the detailed accounts (data-sweep "Vendor bills → GL sub-account" Δ$0.00 + "P&L Actual ← GL rollup" Δ$0.00) — the work is making it VISIBLE + import-time.
- **DEFER the TB storage-overlay** (a restatement engine where a DIFFERING imported TB moves a closed-month Actual). `[DECIDED — defer]` Accuracy comes from the reconciliation control total, NOT from letting imports rewrite Actuals. The overlay would create a 2nd source of truth, silently desync Cash Flow NI from P&L NI (`statements.ts:446` — `niOverlay` collapses to 0 under an activity overlay), need a balanced-month guard, and has zero data-sweep coverage. Stays behind a clean `TBOverlayStore` seam if a real restatement need ever appears.

**Build slices (the reconciliation lives on Setup → Data Import — Chris's call):**
- **1b — the reconcile engine (headless).** `lib/import/reconcile.ts`: per (account, period) for accounts WITH a sub-ledger, `Σ(detail) vs TB movement` → `ReconFinding`/`ReconResult` (already typed in `lib/import/types.ts`). The TB side: export `accountTrialBalanceAt(period): Map<code, naturalAmount>` FROM `lib/seed/gl.ts` (it has `openingByCode` private + the activity map; **P&L = FYTD activity Σ[fyStart..periodIdx], BS = opening + Σ activity[0..periodIdx]** — both natural-balance; reuse the exact opening literal, no dup). The detail side: the sub-ledger sums the data-sweep recons already compute (vendor bills by account/period, invoices, paychecks, receipts). Threshold `max($1, 0.1%·line)`; over → blocking, no plug, fix-upstream. `lib/queries/import.ts` orchestrator: parse → validate → reconcile → if clean AND period === the in-close month → `store.advanceClose(period)` (already built + proven); a clean prior-month re-import is a restatement no-op (green reconcile, as-of unmoved). **Standing tripwire (Loom credibility): extend `scripts/importer-check.ts` so the seed's own books reconcile to $0.00 across every detailed account.** Persist an `import_runs` audit row (migration `0007`, EXCLUDE from the seed-loader clear array; settings → upsert-if-absent so a re-seed never un-advances the as-of — verified breaker at `seed-supabase.ts:82`).
- **2 — the visible Reconciliation surface ON Setup → Data Import (the on-camera piece).** Replace `app/setup/data-import/page.tsx` (PlaceholderPage) with a Server-Action page (mirror the `?edit=` idiom, no client JS): a STANDING reconciliation control total (per account/line: `TB $X · detail $Y · variance $Z`, green when it ties, rolled to each statement line) + a TB upload that runs the same control total + a "Commit & advance as-of" gated on a clean reconcile + the `import_runs` history. Verify live (preview): the books show as reconciled ($0 variance); a broken/contradicting TB lights up the exception list; a clean June TB advances the as-of and June flips to Actual everywhere. §11 unmoved.
- **3 — COA base-import (deferred tier, lower priority).** `chart_of_accounts.csv` re-points `statement_line` at a new `coa_base_import` tier, composed `composeGlAccounts(composeGlAccounts(CHART, coaBaseImport), userOverrides)` (override-check stays 44/44 at empty by reference). Same-group rows auto-apply; cross-group / new-code / accountType-nature changes → BLOCKING needs-attention, never silently dropped.

**New files this session:** `lib/import/{parse-csv,types,validate}.ts` · `scripts/{importer-check,importer-asof-check,flux-repoint-check}.ts`. **HOW TO START 1b:** read this block + `CLAUDE.md` §16/§17; the as-of advance is already built + proven (`importer-asof-check`), so 1b is the detail-to-TB reconcile math (mind the P&L-FYTD vs BS-balance split, and that TB-only accounts have no detail to reconcile) + surfacing it on Data Import. data-sweep MUST stay 44/44; §11 unmoved. The full locked design (3 proposals × adversarial verify × synthesis) is workflow output `wf_e086eecf-da5`.

### State (2026-06-22 · full QC/reconciliation pass + subCode landed — handoff) — GREEN  ·  _prior context_

**A thorough QC + reconciliation of the app/code/data is DONE, the subCode fix LANDED, every agent's test is green, and it's PUSHED.** Local `main` == `origin/main`, 4 commits past the prior push (`ee07169`): `19f79b1` (override layer) · `b3ca5f8` (override-layer handoff) · `cd8e11c` (subCode→Supabase, task_54163c13) · `08899d2` (QC fixes) · + this handoff commit. PUBLIC repo https://github.com/dogsleddev/dogfood.

**QC workflow (6 dimensions × review → adversarial-verify → synthesize; 24 findings raised, 23 confirmed, no P0).** Verdict: the app/code/data are sound and internally consistent — every statement invariant (Net Income, Operating Income, Assets = L+E, **and now Gross Profit**) holds under every allowed override. Fixes applied + committed:
- **F3a [the one behavior change]** — the re-point guard split `pnl_operating_cost` → `pnl_cor` + `pnl_opex`. A CoR↔OpEx re-point (e.g. Direct Payroll → Sales & Marketing) was ACCEPTED and silently moved Gross Profit / Gross Margin % (a reported metric) while the guard docstring claimed the P&L invariant held; it is now REJECTED, so every reported subtotal is invariant under an allowed re-point (mirrors the BS asset/liability section model). **A future CoR↔OpEx in-app reclassification would relax this deliberately + add a GP-moves test (§17 open).**
- **F2** — closed the `account_overrides` Supabase round-trip blind spot (the same class that shipped the subCode bug): `scripts/supabase-parity.ts` now does a write→read→compose→clear assertion against the real backend (`finally`-cleaned, leaves the table empty).
- **F1** — `lib/guides/content.ts` no longer tells users Account Mapping editing is unshipped (it ships): the *updating-actuals* + *reading-the-statements* guides now describe the live editable map (Scout's product-knowledge corpus — was a wrong CFO-facing answer). scout-eval still 64/64.
- Nits: the stale `?? ser[aIdx]` BS comment (a double-count trap for a future maintainer) corrected; the actions.ts error-surfacing comment softened; `InMemoryDataStore.listAccountOverrides` returns a defensive copy; the edit-panel tag copy fixed (tags are descriptive + no longer tells users to cross CoR/OpEx); `supabase/README.md` migration list updated through 0006.
- **`override-check` 37 → 44** (+ CoR→OpEx rejected, + valid CoR→CoR accepted, + monthly-BS closed cells move with Assets=L+E per column, + dashboard byte-identical under a re-point).
- **subCode (task_54163c13) DONE** — `vendor_bills.sub_code` column (`0006_vendor_bill_subcode.sql` + 0001 fold) + `rowToVendorBill` mapper + seed-loader write; migration applied via the Session pooler + Supabase re-seeded; supabase-parity exact (no subCode mismatch).

**Deferred (consciously, recorded):** **F4** — a re-point does NOT move an existing flux note to the new statement line (the note still shows on the account-grain card and the numbers are correct; only the statement-pane filter is stale). Non-numeric, low blast radius; bundle into a focused flux pass (filter account-anchored notes by *effective* statementLineId on read). **F8** — the `openingByCode()` vs `buildLedger.openFor` opening-balance literal is duplicated, but already tripwired by data-sweep at Δ$0.0000; leave it (tie-out-critical area).

**Gates (all green):** tsc/lint 0 · data-sweep **44/44** + BS-rollup tripwire Δ$0.0000 · **override-check 44/44** · scout-readiness 74/74 · scenario-check 20/20 · peer-check §11 unmoved (rev 23,420,700 / NI −11,918,577 / runway 49mo) · **supabase-parity PASS** (+ the new override round-trip) · **scout-eval LLM 64/64** · `next build` 40/40.

**▶ NEXT = the CSV importer** (the original step-6 end goal — now genuinely "wiring": the map reads through the DataStore, so a CSV that rewrites `gl_accounts.statement_line` (base) composes with the user overrides automatically). Then **Scout full scenario writes (`setDriver`** — unblocked since scenario persistence landed), **global as-of Phase B** (Settings write + roll-forward), and the deferred **F4** flux-note-follows-re-point. Lower priority / deferred-by-design: cross-section & cross-statement re-points + the §16 needs-attention flag; the CoR↔OpEx in-app reclassification decision; generalizing the override layer to other synced records. data-sweep MUST stay 44/44; §11 unmoved.

### State (2026-06-22 · Account-Mapping OVERRIDE LAYER — re-pointable map drives P&L/BS/CF Actual) — GREEN  ·  _prior context (the QC block above corrects two now-superseded details: the guard is CoR/OpEx-SPLIT, and the subCode follow-up is DONE)_

**Step 6, slice 2 — the OVERRIDE / PERSISTENCE LAYER — DONE + committed.** The Account Mapping is now EDITABLE and load-bearing across all three statements: a field-level override re-points an account's statement line and the **Actual** columns roll up through the EFFECTIVE map (immutable chart ⊕ overrides). Verified live on Supabase: re-pointing IT→Admin moved the rendered P&L Actual (IT **$171.2K→$0**, Admin **$137.2K→$308.4K**) while Forecast/Budget/Variance stayed put; clean reset. **Chris chose MAXIMAL scope** (re-point moves P&L **and** BS **and** CF + the monthly board views; UI edits statement-line **and** classification/function). Designed via **TWO design+impact workflows** (both critics tieOutSafe **and** identityPreserved). Gates: tsc/lint 0 · data-sweep **44/44** + a new BS-rollup neutrality tripwire (Δ $0.0000) · **override-check 37/37** (new positive gate) · scout-readiness 74/74 · scenario-check 20/20 · peer-check §11 unmoved (rev 23,420,700 / NI −11,918,577 / runway 49mo) · `next build` OK · supabase-parity `gl_accounts` exact.

- **Architecture (empty-override path byte-identical by construction):** discrete `account_overrides` table keyed by `code` (NO hard FK, excluded from the seed-loader clear array → survives re-import); `composeGlAccounts(base, overrides)` returns base **by reference** when empty. Read path = **parameter injection, `lib/seed/*` stays sync+pure**: `activityByStatementLine(accounts?)` + new `balanceSeriesByLine`/`balanceByStatementLine` (opening-by-line + cumulative activity — the BS twin); the P&L/BS/CF + monthly builders take `accounts?` defaulting to `CHART_OF_ACCOUNTS`; the DataStore getters `await listGlAccounts()` and thread it. `SupabaseDataStore` inherits (overrides ONLY `listGlAccounts` + the 3 override writes) → **closes the §17 split-brain** (statements now read the same map `listGlAccounts()` returns).
- **The hard parts (resolved):** BS Actual rolls up with an **`?? 0`** fallback (an uncovered line shows 0, NOT its series — so the section sum + **Assets = L+E hold on the actual column** under a same-section re-point; a series fallback would double-count). CF working-capital deltas read the SAME `balanceSeriesByLine` (single source, no double-derivation); **CF Net Income uses a delta-overlay** (`flow(s.netIncome)` base + `rolledNI − rolledNI-at-base`, = 0 on the empty path → no new rounding, byte-identical; moves coherently on a re-point). Monthly board cells roll up for CLOSED months only; **totals stay forecast/series-driven** so ties 204/207/221 are untouched.
- **Guard (query layer):** re-point ONLY within a group (P&L revenue↔revenue, operating-cost↔operating-cost; BS asset↔asset, liability↔liability) → NI + Assets=L+E invariant; rejects subtotals, equity, below-the-line, cross-statement (override-check assertion 6). `classification`/`function` are editable **descriptive tags** (no number reads them) with a "doesn't match the statement line" warning. **Cross-section/cross-statement re-points + the §16 needs-attention flag = DEFERRED** (unanimous across 6 critics; the app can't render a deliberately-imbalanced sheet). **Scenario(Base) Actual reads the immutable base map this slice** (§9 containment, documented).
- **UI:** `?edit=<code>` side panel (Server Actions, no client JS) mirroring the `?note=` idiom; statement-line + class/function selects; Reset; per-row "edited" badge. Files: `lib/types/source.ts` (AccountOverride), `lib/datastore/account-overrides.ts` (compose), `lib/datastore/{datastore,in-memory,supabase}.ts`, `lib/seed/{gl,statements}.ts`, `lib/queries/account-mapping.ts`, `app/setup/account-mapping/{page,actions}.tsx`, `components/setup/{account-mapping-table,account-edit-panel}.tsx`, `scripts/override-check.ts`, `scripts/data-sweep.ts`, `supabase/migrations/0005_account_overrides.sql` (applied via the pooler) + 0001 fold.
- **Supabase was STALE → RE-SEEDED this session.** A pre-existing condition (the expense-granularity/period-seam/rollup commits regenerated the seed after the 2026-06-21 load, but Supabase was never re-seeded): supabase-parity showed count mismatches on vendor_bills/timesheets. Re-ran `scripts/seed-supabase.ts` → counts now match; `gl_accounts` exact. **`account_overrides` table created in Supabase (empty).**
- **⚠ FLAGGED follow-up (task_54163c13) — `vendor_bills.subCode` does NOT round-trip to Supabase.** The expense-granularity commit (a17f1e2) added `subCode` to the generator/type but never added a `sub_code` column to the Supabase schema, the `rowToVendorBill` mapper, or the seed-loader insert. So supabase-parity still shows **2 MISMATCH** (vendor_bills + expenses-2026-05, differing ONLY in `subCode`), and the Expense Forecast sub-account drill's closed-month breakdown is wrong on the Supabase backend. Orthogonal to the override layer (gl_accounts is parity-exact). Fix = migration `sub_code` column + mapper + loader + re-seed.

**▶ NEXT = the CSV importer (the original step-6 end goal — now genuinely "wiring" on the load-bearing + override-aware map), and/or the `subCode`-Supabase fidelity fix (task_54163c13, makes supabase-parity fully green).** Then the other open write paths: **Scout full scenario writes (`setDriver`** — unblocked since scenario persistence landed), **global as-of Phase B** (Settings write + roll-forward + the deferred forecast-month sub-ledger cut). Lower priority / deferred-by-design: cross-section & cross-statement re-points + the §16 needs-attention flag; generalizing the override layer to other synced records; the monthly-board NI-cell overlay (currently series-only — a documented corner). **HOW TO START:** read this block + `CLAUDE.md` §4/§7/§16/§17 + the build-state memory; for the importer, the map is now read through the DataStore so a CSV that re-writes `gl_accounts.statement_line` (base) composes with the user overlays automatically. data-sweep MUST stay 44/44; §11 unmoved.

### State (2026-06-22 · expense granularity + period seam + Account-Mapping rollup) — GREEN  ·  _prior context_

**Continued the resequenced plan + a user-requested feature. Everything below is committed AND pushed; `origin/main` == local at `0ee205d` (PUBLIC repo, https://github.com/dogsleddev/dogfood).** Gates green throughout: tsc/lint 0 · data-sweep **44/44** (+ two independent reconciliation prints — "Vendor bills → GL sub-account" and "P&L Actual ← GL rollup", both Δ$0.00) · scout-readiness 74/74 · scenario-check 20/20 · scout-eval structural+deterministic + LLM **64/64** · `next build` 40/40 · peer-check §11 unmoved (rev **23,420,700** / NI **−11,918,577** / NRR 109 / magic 1.2x / LTV 4.1x / runway 49mo).

**Commits since the steps-1–4 block (all pushed):**
- **Expense Forecast granularity (group→account→vendor)** — `ef26712` (seed+query), `f3f3a76` (forecast-page drill UI), `a17f1e2` (P&L peek pane + Scout + export/sweep). Chris: "S&M July $268,650 was a black box." Sub-accounts are a CONFIG layer with display codes (6210/6220…), **NOT new GlAccounts** — CHART_OF_ACCOUNTS/GL/statement-engine byte-identical, group total unchanged → §11 untouched. `lib/seed/opex-accounts.ts` (29 sub-accounts, Σshares===1.0/group, asserted at load); `getExpenseForecast(period,{groupId,breakdown:'group'|'account'|'vendor'})` (forecast = anchors at run-rate + an "Other —" residual; closed months = real bills by subCode); UI on the `/forecasts/expenses/[group]` page (account×month grid + month-chip vendor tree) + the P&L peek pane "By account" + Scout `breakdown` arg.
- **Step 5 — period seam** — `c58d02f`. Centralized the period math into `lib/types/period.ts` (module-private `FY_BASE_YEAR`, `monthToIndex`, `indexToMonth` moved from params + re-exported, `fyStartIndex`/`fyStartIndexForYear`); replaced ~12 local dupes; **fixed `renewals.ts` NOW_IDX hardcoded 29 → `monthToIndex(inCloseMonth)` (June=29, not closeThrough May=28)**. Numbers-neutral. LEFT the closeThrough SOURCE in PLACEHOLDER_SETTINGS (the global-as-of step is still future) and `fyWindow`/`seedFyWindow` in place.
- **Step 6 SLICE 1 — Account Mapping load-bearing** — `0ee205d`. The P&L ACTUAL column now rolls up from GL account activity via the `statementLineId` map (`activityByStatementLine()` in `lib/seed/gl.ts`; `computeColumns` + `computeColumnsFromSeries` read it for the Actual term, over the same `[fyStart..actualEnd]` window; Forecast+Budget stay driver-driven). Was decorative. Tie-out-neutral (Option A, compute-only). Designed via a 6-agent design+impact workflow (output `wn1m471ix`); both critics returned tieOutSafe. New data-sweep recon proves it at the closed-window grain.

**▶ NEXT (Chris's pick) = the OVERRIDE / PERSISTENCE LAYER (step 6, next slice). Run a design+impact workflow FIRST (like the rollup + expense feature got) — it is architecturally significant.** It is the foundation for two things:
- **The editable Account-Mapping write UI** — re-point a mapping in the browser and watch the P&L Actual move (the *visible* payoff of slice 1). Today the rollup reads the GENERATOR chart (`CHART_OF_ACCOUNTS`), so a user/DB edit moves nothing — **the rollup must read the map through the DataStore (`SupabaseDataStore`), not the generator constant.** That coupling IS the override layer.
- **The CSV importer** (the original step-6 end goal) — now "wiring" on the load-bearing map.
- **Decided model (§17):** the override layer = **field-level deltas** on synced/immutable records, surviving re-import — the same immutable-source + user-delta pattern as `scenario_inputs` and `flux_notes`, persisted in Supabase.
- **Other step-6 slices still open** (lower priority, flagged in workflow `wn1m471ix`): BS actual rollup (opening + Σactivity through aIdx, re-key `openFor` by `statementLineId`) → then CF actual falls out; the monthly-board actual-month values; fold `OPEX_GROUP_TO_ACCOUNT` into `ACCOUNTS` via an `expenseGroupId` back-ref.
- **HOW TO START:** read this block + `CLAUDE.md` §4/§7/§16/§17 + the build-state auto-memory; run a design+impact workflow scoping the override layer (where deltas persist in Supabase; how the rollup + record reads route through the DataStore so a map/field edit moves the statement; how re-import preserves overrides; the Account-Mapping write UI + Server Action). Implement against the tie-out gate (data-sweep MUST stay 44/44; §11 unmoved).

**GOTCHAS (recurring — read before pushing or starting the dev server):**
- **`git push` hangs with no output** → the Git Credential Manager credential expires (~1hr) and its GUI helper hangs in this headless shell (`GIT_TERMINAL_PROMPT=0` does NOT stop the GUI helper). FIX: `gh auth setup-git` (gh is authed as dogsleddev, keyring) wires gh's token as git's credential helper; then `git push` works non-interactively. Kill stray `git`/`git-remote-https` procs first; never launch concurrent pushes.
- **`getDataStore()` is a globalThis-cached singleton** → after ADDING DataStore methods, RESTART the dev server (the cached instance predates them → "X is not a function"; HMR alone won't fix it).
- **`next build` while the dev server runs corrupts `.next`** (`_buildManifest.js.tmp` ENOENT → Internal Server Error on every page) → stop the dev server before a build; clear `.next` + restart dev after a build.
- **Seed edits shift the shared RNG** → vendor/paycheck/timesheet/receipt TEXTURE changes (tie-out-neutral); always re-run data-sweep + peer-check after seed-area edits.
- **Commit messages:** use `git commit -F <file>` (PowerShell here-strings break on inline `"`/`->`).

### State (2026-06-22 · resequenced-plan steps 1–4: quick-wins + monthly BS/CF + budget lock + seed-narrative pass) — GREEN  ·  _prior context_

**Executing the 7-reader review's RESEQUENCED order (Chris approved "follow resequenced plan"). Resequenced step numbering ≠ the original ▶ NEXT list further below.** Steps 1–4 done + committed + **pushed** (`879f039..f8c33e7`; `origin/main` == local, PUBLIC repo). Gates green throughout: tsc/lint 0 · data-sweep 43/43 · scout-readiness 74/74 · scenario-check 20/20 · scout-eval structural+deterministic PASS + LLM 63/63 · `next build` 40/40. Verified live in the preview each step.

- **Step 1 (38e96d4) — quick-win batch.** `assertBaseScope` containment guard (a non-null `scenarioId` now THROWS on every Base read — 5 statements + dashboard + 7 drivers — it was silently dropped); fixed the stale ERP "single live integration" string (data-import page + `erp-connector.ts`); softened the dashboard pure-metric "decompose in a pane" copy (no `?inspect` pane exists on the dashboard); implemented the two real `getGrr`/`getPipelineCoverage` sales stubs (the original step-12 housekeeping — GRR 86.1% matches getRenewals, coverage 1.7x); `addFluxNote` receipt href is now anchor-aware (account notes → `/setup/account-mapping?note=`, the step-12 href nit); added a REAL `cac_payback_months` tile (15 mo) keeping the honest dollar `cac_payback` → 20 dashboard tiles.
- **Step 2 (3939c94) — monthly Balance Sheet + Cash Flow views** (review-surfaced; not in the original list). Cloned the P&L FY/Monthly toggle; BS Total col = FY-end balance (point-in-time), CF telescopes (MoM working-capital deltas). New `getMonthlyBalanceSheet`/`getMonthlyCashFlow` Scout tools → 36 tools; caught + fixed a deterministic-router collision (the flux "break down" route shadowed the monthly-CF route).
- **Step 3 (f8c33e7) — budget lock (= the original ▶ NEXT step 6).** Real frozen `BudgetSnapshot` replaces the synthetic per-read `forecast × factor`. `applyBudgetSnapshot()` overlays a locked snapshot's per-line budget onto the immutable forecast/actual P&L (keyed by id, leaves + subtotals, no re-foot); the factor stays only as the default-plan generator + prior-FY fallback. DataStore `lockBudget`/`resetBudget`; in-memory lazy-seeds the FY26-Plan lock + `getPnL` overlays it; SupabaseDataStore inherits it. `BudgetStatus` strip + `resetBudgetAction` on the Forecasted P&L. **Decision (Chris): freeze CURRENT values (Δ=0.00, §11 numbers unchanged) but resettable.** GOTCHA: `getDataStore()` is a globalThis singleton → after ADDING DataStore methods, **restart the dev server** (the cached instance predates them → "X is not a function"). Follow-up (non-blocking): persist `budget_snapshots` to the Supabase table (today in-process + deterministically reconstructible, so identical on both stores).
- **Step 4 — seed-narrative pass (this block + the doc edits).** Re-anchored CLAUDE.md §0/§8/§10/§11 to the live numbers (LTV:CAC 4.3x→4.1x, NI −52%→−51%, tools 34→36, eval 58/58→63/63, scout-readiness 34/34→74/74, 19→20 tiles, all three statements now monthly), and led §11 with the "why the ~49-month runway is structural (Bearing is annual-prepay → growth generates cash)" defense (§17 dated notes left as historical record). Fixed two truthfulness gaps in Scout's guide corpus: the budget-lock framing is now live (lock + reset), and BS/CF now have monthly toggles too.

**▶ NEXT = resequenced step 5 (period seam):** centralize `closeThrough` + unify `monthToIndex` into `lib/types/period.ts` (the as-of debt is ~38 files / 53 sites + magic `NOW_IDX=29` in `renewals.ts`) — a prerequisite for the global as-of, kept OUT of the importer diff. Route pages/Scout; LEAVE the tie-out-critical seed builders on the sync constant. Then resequenced step 6 (the Account-Mapping GL→statement-line rollup + the override layer + the CSV importer, option a), then Scout full scenario writes (`setDriver`, now unblocked by scenario persistence), as-of Phase B, and a fuller guide-alignment pass. **The original ▶ NEXT list below maps onto this:** its step 6 (budget lock) = DONE; its step-12 stubs/href = DONE in step 1; 7 / 8 / 9-10 / 11 remain.

### State (2026-06-21 · next-steps review + steps 1–5: truthful guides + edge access gate + decisions + scenario store unified + scenario persistence) — GREEN  ·  _prior context_

**A grounded next-steps review ran (a 7-reader verification workflow over the remaining workstreams) and Chris approved the recommended order. Steps 1–5 of that order are done this session** — steps 1–3 are detailed immediately below; steps 4–5 (scenario store unification + full scenario persistence, with an adversarial review) are in the **▶ NEXT** block and their own commits. **Everything is committed and pushed to https://github.com/dogsleddev/dogfood (PUBLIC); local == `origin/main`.** All commits are authored as Chris &lt;dougherty4@gmail.com&gt;. **Next is step 6 (budget lock).** Gates green throughout: tsc/lint/`next build` 0 · data-sweep · scout-readiness 70/70 · scenario-check **20/20 (in-memory AND live Supabase)** · supabase-parity.
...
**▶ NEXT — steps 4–5 DONE (2026-06-21); continue from step 6.** Corrected build order (the split-brain finding was new — the handoff had framed scenario persistence as "just wire setDriver"): **(4) DONE** — split-brain scenario store unified: `lib/scenario/registry.ts` is now async + DataStore-backed (`allScenarios`/`findScenario` compose the code-defined Base + presets with `getDataStore().listScenarios()` user scenarios; `upsert`/`delete` reject the immutable Base/preset ids), `lib/queries/scenarios.ts` awaits it. **(5) DONE** — scenario persistence: `scenarios.adjustments` JSONB (migration 0004, applied live + folded into 0001; flat `scenario_inputs` left orphaned); `SupabaseDataStore` scenario overrides + a lossless typed serializer (parseAdjustment/parseMagnitude, re-brands + fails loud); Manager (create/duplicate/reset/delete) + Drivers (add/remove) Server Actions + the authoring UI (no client JS); validate-before-save against the live horizon; the magnitude-kind is DERIVED from the lever (+ a `magnitude_kind_mismatch` validation rule) so no zombie adjustments. **Adversarial review done** (1 HIGH + 3 MED + lows all fixed; only the latent duplicate/create horizon-revalidation deferred to the as-of work). Verified live: create→persist→edit→delete end-to-end (0 console errors); scenario-check **20/20 BOTH in-memory AND DATASTORE=supabase**; tsc/lint/build 0; data-sweep, scout-readiness 70/70. **(6)** budget lock (real snapshot + Lock UI; re-run data-sweep — Budget/Variance numbers move). **(7)** Scout full scenario writes (create+setDriver, relax the WHATIF guard, update `config.ts`). **(8)** global as-of Phase A (centralize `closeThrough` + unify `monthToIndex` into `period.ts`; the debt is ~38 files / 53 sites + magic `NOW_IDX=29` in `renewals.ts`, not "~12"; route pages/Scout, LEAVE the tie-out-critical seed builders on the sync constant). **(9/10)** override layer (field-level) + CSV importer (option a) together. **(11)** as-of Phase B (Settings write + roll-forward + the deferred forecast-month sub-ledger cut #15). **(12)** housekeeping (`getPipelineCoverage`/`getGrr` stubs at `lib/queries/sales.ts:34,201`; missing `cost-and-pl.svg` / `architecture.html`; the `addFluxNote` backslash-href nit at `lib/scout/tools.ts:~870`).
