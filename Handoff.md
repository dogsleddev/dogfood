...
**New files:** `lib/import/{reconcile,sample-tb}.ts` · `lib/queries/import.ts` · `components/setup/reconciliation-table.tsx` · `app/setup/data-import/{page,actions}.tsx` · `supabase/migrations/0007_import_runs.sql` · `scripts/{make-trial-balance,reset-asof}.ts` · `import-templates/trial_balance_{next-month,broken}.csv`. **Changed:** `lib/seed/gl.ts` · `lib/import/types.ts` · `lib/datastore/{datastore,in-memory,supabase}.ts` · `scripts/{importer-check,seed-supabase,supabase-parity}.ts` · `components/nav/nav-config.ts` · `import-templates/trial_balance.csv` (regenerated) · **Scout tool:** `lib/queries/registry.ts` · `lib/scout/{tools,config,router,commentary}.ts` · `scripts/scout-eval.ts` · `lib/guides/content.ts`.

**⚠ STATE — this session is UNCOMMITTED (held on `main` with the prior 3 local-only commits `c79e78d` / `36abe20` / `5397c1e`).** Nothing pushed. **TODO for Chris:** (1) **apply migration `0007` to Supabase** (Session pooler, like prior migrations) for durable import history — the graceful fallback works without it, but a server restart drops the in-memory runs; (2) the **advance demo persists the as-of to Supabase** (June) — `npx tsx scripts/reset-asof.ts` rolls it back to May; (3) commit + push when you OK (`gh auth setup-git` first — the credential-hang gotcha).
