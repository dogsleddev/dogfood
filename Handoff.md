...
4. **Budget realism** -- add 2-3 deliberate, narratable budget variances, or is the synthetic +/-3% jitter fine for the demo?
5. **Scout scenario scope** -- can Scout `createScenario`, or only `setDriver` on the current one? (Decide before wiring scenario tools.)
